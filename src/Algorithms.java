...
public class Algorithms {
    ...
    // Solves the best path in the maze
    public static ArrayList<KObjs> solveMaze(Maze m) {
        ArrayList<KObjs> klist = new ArrayList<KObjs>();
        ArrayList<KObjs> solved = new ArrayList<KObjs>();
        klist.add(new KObjs(new Posn(0, 0), "west"));
        solved.add(new KObjs(new Posn(0, 0), "west"));
        KObjs current = klist.get(0);
        boolean bcheck;
        KObjs test;
        while (!current.pos.equals(m.rooms.get(m.rooms.size() - 1).coord)) {
